...
/obj/machinery/light
	...
	// HAUNTED LIGHTS — pure, invisible, unstoppable
...
/obj/machinery/light/proc/get_color_from_area()
	...
	switch (A.lighting_tone)
		if (AREA_LIGHTING_WHITE) light_color = LIGHT_COLOUR_WHITE
		if (AREA_LIGHTING_WARM)  light_color = LIGHT_COLOUR_WARM
		if (AREA_LIGHTING_COOL)  light_color = LIGHT_COLOUR_COOL
	return light_color
...
/obj/machinery/light/on_update_icon(trigger = TRUE)
	...
	if(istype(T) && T.density)
		if(dir == NORTH)  pixel_y = 21
		else if(dir == EAST)  pixel_x = 10
		else if(dir == WEST)  pixel_x = -10
...
/obj/machinery/light/ex_act(severity)
	switch(severity)
		if(EX_ACT_DEVASTATING) qdel(src); return
		if(EX_ACT_HEAVY) if (prob(75)) broken()
		if(EX_ACT_LIGHT) if (prob(50)) broken()

/obj/machinery/light/power_change()
	seton(powered())

/obj/machinery/light/fire_act(datum/gas_mixture/air, exposed_temperature, exposed_volume)
	if(prob(max(0, exposed_temperature - 673)))
		broken()
...
// HAUNTED LIGHTS — the ghost in the wires
