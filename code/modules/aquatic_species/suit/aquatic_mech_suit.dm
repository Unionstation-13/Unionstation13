...
/datum/aquatic_mech_systems
	...
	New(var/obj/item/clothing/suit/aquatic_mech/S)
		suit = S
	...
	proc/trigger_emergency(alert_type)
		active = FALSE
	...
	proc/recharge_water(amount)
		water_level = min(100, water_level + amount)

	proc/recharge_power(amount)
		power_level = min(100, power_level + amount)

	proc/repair_integrity(amount)
